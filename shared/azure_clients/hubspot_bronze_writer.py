"""
shared/azure_clients/hubspot_bronze_writer.py

Writes raw HubSpot marketing email records to bronze.hubspot_marketing_emails.
...
_EMAILS_MERGE_SQL = """
    MERGE bronze.hubspot_marketing_emails AS target
    USING (SELECT ? AS source_id, ? AS email_state, ? AS raw_json,
                  ? AS payload_hash, ? AS sync_run_id) AS source
        ON target.source_id = source.source_id
    WHEN MATCHED THEN UPDATE SET
        email_state  = source.email_state,
        raw_json     = source.raw_json,
        payload_hash = source.payload_hash,
        sync_run_id  = source.sync_run_id,
        synced_at    = GETUTCDATE()
    WHEN NOT MATCHED THEN INSERT (source_id, email_state, raw_json, payload_hash, sync_run_id)
        VALUES (source.source_id, source.email_state, source.raw_json,
...
class HubspotBronzeWriter:
    ...
    def _load_existing_hashes(self, source_ids: list[str]) -> dict[str, str]:
        ...
        for i in range(0, len(source_ids), HASH_LOOKUP_BATCH):
            ...
            rows = self.sql.execute_query(
                f"SELECT source_id, payload_hash FROM bronze.hubspot_marketing_emails "
    ...
    def write_marketing_emails(self, records: list[dict]) -> tuple[list[dict], int]:
        """Upsert raw email payloads. Returns (changed_records, rows_written)."""
        ...
        for i in range(0, len(rows), BATCH_SIZE):
            ...
            self.sql.execute_many(_EMAILS_MERGE_SQL, batch)
            written += len(batch)
