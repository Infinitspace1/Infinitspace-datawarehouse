...
class TestXeroIntegration(unittest.TestCase):
    ...
    def test_get_invoices_uses_invoice_endpoint_and_page(self):
        ...
        store._connection = StoredXeroConnection(
            ...
            refresh_token="refresh-ok",
            id_token=None,
            scope="offline_access accounting.contacts accounting.transactions.read",
            token_type="Bearer",
            expires_at=datetime.now(timezone.utc) + timedelta(minutes=60),
            selected_xero_tenant_id="tenant-xyz",
            is_connected=True,
    ...
    def test_get_accounts_uses_accounts_endpoint(self):
        ...
        store._connection = StoredXeroConnection(
            ...
            refresh_token="refresh-ok",
            id_token=None,
            scope="offline_access accounting.contacts accounting.transactions.read accounting.settings.read",
            token_type="Bearer",
            expires_at=datetime.now(timezone.utc) + timedelta(minutes=60),
            selected_xero_tenant_id="tenant-xyz",
            is_connected=True,
