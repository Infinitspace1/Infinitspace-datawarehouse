...
def _raw(**overrides) -> dict:
    base = {
        ...
        "country": "Ireland",
        "zipCode": "D01 X1X1",
        "photoUrl": "https://photo.example.com/42.jpg",
    }
...
class TestTransformEmployee(unittest.TestCase):
    ...
    def test_empty_string_date_returns_none(self):
        result = transform_employee(_raw(terminationDate=""), bronze_id=1, sync_run_id="x")
        self.assertIsNone(result["termination_date"])

    def test_bamboohr_zero_date_returns_none(self):
        result = transform_employee(_raw(terminationDate="0000-00-00"), bronze_id=1, sync_run_id="x")
        self.assertIsNone(result["termination_date"])
