...
| AVA location plans | done (code) | `ava.location_plans` (per-location plans = filtered `silver.nexudus_tariffs`; excludes desks/offices via `SystemTariffType` 1/3/5 + €0 plans); SP `ava.sp_refresh_location_plans`, wired into `refresh_ava_availability`; schema `scripts/sql_scripts/ava_location_plans_schema.sql` **not yet applied to prod** (silver layer unchanged) |
