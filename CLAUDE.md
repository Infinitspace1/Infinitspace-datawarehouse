...
| Location Scraper (Idealista + Otodom + IS24 + LoopNet) | done | HTTP-triggered Durable Functions pipeline; `ENABLE_LOCATION_SCRAPER_FUNCTIONS=1`; Idealista (ES/IT) + Otodom (PL) + Immobilienscout24 (DE) + LoopNet (UK/London + US: New York, San Francisco, Palo Alto, Los Angeles, Austin, Seattle, Redwood City, San Mateo, San Bruno, Cupertino, Phoenix, Atlanta); LoopNet uses a 2-step full-coverage flow (enumerate filtered search via abotapi → memo23 per listing URL, ~383 London buildings vs 42 with the old capped broad search); Lusha enrichment via fan-out; free Nominatim geocode fallback when no `GOOGLE_MAPS_API_KEY`; bronze schema; see `docs/location_scraper.md` |
