...
| `xero_invoice_sync` | `functions/xero_sync.py` | timer | `0 0 4 * * *` | syncs all linked Xero tenants + caches PDFs for invoices missing `pdf_blob_path`; reuses the backfill retry/throttle flow; then syncs bank transactions (spend/receive money) with a graceful skip for tenants missing the `accounting.banktransactions` scope |
...
| Xero bank transactions sync | done (code) | **Blocked on OAuth re-consent** — the stored token lacks `accounting.banktransactions[.read]` (granted scope is invoices+settings only), so every tenant is skipped gracefully with a warning until someone with access to all 8 orgs re-consents with the full `.env.example` scope list. Code + schema + tests + local harness ready; deploy = push to `main`, then apply `xero_bank_transactions_schema.sql`; first post-consent run auto-backfills. Serving view for the budget tool: `silver.vw_xero_bank_transaction_pnl_lines` (net-of-tax, SPEND/RECEIVE only) |
