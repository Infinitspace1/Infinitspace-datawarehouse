...
- Nexudus API (incl. events: calendar events, attendees, ticket products;
  and help desk: customer requests, reply threads, departments)
- Xero API
...
13. `every 15 min` `nexudus_helpdesk_sync` (help desk: bronze + silver inline; see below)
14. `on demand` `nexudus_helpdesk_webhook` (HTTP; Nexudus pushes actions 45/46)
15. `10:00` `refresh_finance_dashboard_recheck` (re-runs the same guarded rebuild after the early-morning data settles, so an under-built 05:30 worklist is republished in full)
...
Nexudus help desk  (HYBRID: push + reconciling poll)
  -> nexudus_helpdesk_webhook (HTTP POST /api/nexudus/helpdesk-webhook)
       Nexudus webhook actions 45 HelDeskMessageCreated / 46 HelpDeskCommentCreated
       -> re-fetches the canonical record by ID (shape parity with the poll)
       -> bronze -> silver, seconds after the ticket is raised
  -> nexudus_helpdesk_sync (timer, every 15 min)
       GET /api/support/helpdeskdepartments   (from_HelpDeskDepartment_UpdatedOn)
       GET /api/support/helpdeskmessages      (from_HelpDeskMessage_UpdatedOn)
       GET /api/support/helpdeskcomments      (from_HelpDeskComment_UpdatedOn)
       -> bronze.nexudus_helpdesk_{messages,comments,departments}
       -> silver.nexudus_helpdesk_{messages,comments,departments}
       NB the poll is NOT optional: Nexudus has no update/close/assign webhook
       event, so ticket lifecycle only ever arrives by re-reading.
...
Infinitspace-datawarehouse/
  ...
  functions/
    ...
    silver_worker.py
    ava_refresh.py
    nexudus_helpdesk_sync.py
    nexudus_helpdesk_webhook.py
  ...
  shared/
    azure_clients/
      ...
      silver_writer_helpdesk_messages.py
      silver_writer_helpdesk_comments.py
      silver_writer_helpdesk_departments.py
    ...
    nexudus/
      auth.py
      client.py
      helpdesk.py            (endpoint/filter registry, webhook routing + signature, hook health)
      ...
      transformers/
        ...
        event_attendees.py
        event_products.py
        helpdesk_messages.py
        helpdesk_comments.py
        helpdesk_departments.py
    bamboohr/
      __init__.py
      client.py
      transformers/
  ...
  scripts/
    python_scripts/
      ...
      test_helpdesk_sync.py                       (Nexudus help desk: dry-run / --write / --write --full backfill)
      inspect_nexudus_helpdesk.py                 (help-desk endpoint discovery, field shape, volumetry)
      inspect_nexudus_helpdesk_fit.py             (incremental filters, thread linkage, silver join coverage)
    ...
    sql_scripts/
      ...
      nexudus_helpdesk_schema.sql                   (bronze + silver: helpdesk_messages, helpdesk_comments, helpdesk_departments)
...
| `nexudus_helpdesk_sync` | `functions/nexudus_helpdesk_sync.py` | timer | `0 */15 * * * *` | help desk (customer requests): bronze + silver inline for messages/comments/departments. Incremental via the Nexudus-native `from_{Entity}_UpdatedOn` filter over a 24h lookback (`NEXUDUS_HELPDESK_LOOKBACK_MINUTES`) — **`UpdatedSince` is silently ignored by these endpoints**. Also calls `check_webhook_health` so an auto-disabled hook surfaces within 15 min |
| `nexudus_helpdesk_webhook` | `functions/nexudus_helpdesk_webhook.py` | HTTP POST | `nexudus/helpdesk-webhook` | Nexudus push for actions 45 `HelDeskMessageCreated` / 46 `HelpDeskCommentCreated`. Re-fetches the canonical record by ID, writes bronze, promotes silver. Answers 200 on almost everything on purpose (see notes below); auth_level=FUNCTION |
...
| `location_scraper_http` | `functions/location_scraper.py` | HTTP POST | `/api/api/scrape` | only when `ENABLE_LOCATION_SCRAPER_FUNCTIONS=1` — Durable Functions starter; returns 202. **Actual path is `/api/api/scrape`** (default `routePrefix` `api` + `route="api/scrape"`); body `{"City":"London"}` (+ optional `run_id`), auth via `x-functions-key` header or `?code=` host key |
...
- `bronze.nexudus_helpdesk_messages` — `location_id`/`coworker_id`/`department_id` denorms
- `bronze.nexudus_helpdesk_comments` — `helpdesk_message_id`/`coworker_id` denorms (payload has no BusinessId)
- `bronze.nexudus_helpdesk_departments` — `location_id` denorm
...
- `silver.nexudus_helpdesk_messages` — the customer requests ("tickets"): `location_source_id` → locations; `coworker_source_id` → coworkers (the requester); `department_source_id` → helpdesk_departments (NULL on ~3%); subject/message_text, priority, `is_closed`/`closed_on`, owner (a Nexudus user, not a coworker), `first_response_minutes`, and **`minutes_to_close` derived by us** — the source `MinutesToClose` is computed backwards by Nexudus and returns a NEGATIVE number, so it is deliberately not copied; reconciled weekly
- `silver.nexudus_helpdesk_comments` — the reply thread: `helpdesk_message_source_id` → messages; `location_source_id` inherited from the parent message by the silver writer (payload has no BusinessId, same trick as event_products); `coworker_source_id` is the AUTHOR (staff replies carry one too — use `is_internal`/`updated_by` to tell staff from member); reconciled weekly
- `silver.nexudus_helpdesk_departments` — routing categories, **per location**, so the same name (e.g. "Air con queries") legitimately exists once per site. Never group by name alone — group by `source_id` or `(location_source_id, name)`; reconciled weekly
...
| `nexudus_helpdesk_messages` | `nexudus_silver_reconcile` | weekly Sun 01:00 | ≤7d |
| `nexudus_helpdesk_comments` | `nexudus_silver_reconcile` | weekly Sun 01:00 | ≤7d |
| `nexudus_helpdesk_departments` | `nexudus_silver_reconcile` | weekly Sun 01:00 | ≤7d |
...
NEXUDUS_HELPDESK_SYNC_SCHEDULE="0 */15 * * * *"           # help desk reconciling poll
NEXUDUS_HELPDESK_LOOKBACK_MINUTES=1440                    # UpdatedOn window each poll re-reads (self-healing)
# NEXUDUS_WEBHOOK_SECRET=...                              # optional HMAC shared secret for the push endpoint
NEXUDUS_WEBHOOK_SIGNATURE_MODE=warn                       # off | warn | enforce (algorithm is undocumented — start at warn)
NEXUDUS_WEBHOOK_TIMEOUT_SECONDS=10                        # ceiling on the Nexudus round-trip inside the webhook handler
...
Nexudus help desk validation (customer requests):

```powershell
# 1. Apply schema once (already applied to prod 2026-08-20)
.env\Scripts\python.exe scripts\python_scriptspply_schema_script.py scripts/sql_scripts/nexudus_helpdesk_schema.sql
# 2. Unit tests (no creds / no DB) — transformers, webhook routing, signature, MERGE parity
.env\Scripts\python.exe -m unittest tests.test_nexudus_helpdesk
# 3. Dry run — fetch + transform samples, and assert the incremental filter still filters
.env\Scripts\python.exe scripts\python_scripts	est_helpdesk_sync.py
# 4. Incremental run — the steady state (~21 rows fetched, ~0 written)
.env\Scripts\python.exe scripts\python_scripts	est_helpdesk_sync.py --write
# 5. Full backfill — ignores the incremental window
.env\Scripts\python.exe scripts\python_scripts	est_helpdesk_sync.py --write --full
# Endpoint discovery / field-shape re-verification against the live API
.env\Scripts\python.exe scripts\python_scripts\inspect_nexudus_helpdesk.py
.env\Scripts\python.exe scripts\python_scripts\inspect_nexudus_helpdesk_fit.py
```
...
| Nexudus help desk (customer requests) | done | **Hybrid ingestion, live-ish by design.** 3 entities via `support/helpdeskmessages` / `helpdeskcomments` / `helpdeskdepartments` (API area is `support`, NOT `community`). Push: `nexudus_helpdesk_webhook` for Nexudus actions 45/46. Reconcile: `nexudus_helpdesk_sync` every 15 min — **required**, because Nexudus has no update/close/assign event anywhere in its 90-code enum. Schema applied + backfilled to prod 2026-08-20: **2,840 messages / 4,293 comments / 75 departments**; joins 2840/2840 locations, 2828/2840 coworkers, 4293/4293 comment→ticket. **Webhook not yet registered in Nexudus** — needs the app settings + one config change in the Nexudus admin panel (see changelog) |
...
Last updated: 2026-08-31 (**Landlord occupancy switched to DAY-WEIGHTED basis (desk-days), applied to prod — past months deliberately NOT restated.** Decision (Bryan): occupancy was gameable (a tenant starting the 28th earned the whole month, feeding occupancy-bonus and landlord disputes) and irreconcilable with the invoice-pro-rated revenue. New basis = the Budget Tracker's exact formula: `occupancy_pct = occupied desk-days / (total_workstation_capacity × days in month)`; per contract the occupied window uses the +4h-shifted start (inclusive) and the **+4h-shifted cancellation EXCLUSIVE** (a cancellation stored "D 22:00 UTC" shifts to D+1 → occupied through D); contracts starting AND ending inside the same month DO count (the Tracker's own known blind spot, fixed here); denominator is the EXISTING monthly binary capacity (NOT day-weighted). **Only the percentage changed basis** — revenue, `occupied_workstations` (end-of-month position), vacant, flow columns all keep the month-end rule. Changes: `gold.vw_landlord_membership_book_monthly` ([landlord_dashboard_revenue_schema.sql](scripts/sql_scripts/landlord_dashboard_revenue_schema.sql)) + `gold.vw_landlord_contract_book_monthly` ([landlord_dashboard_schema.sql](scripts/sql_scripts/landlord_dashboard_schema.sql)) gain a `desk_days_by_month` CTE (`effective_cancel_excl` added to contract_facts), swap `occupancy_pct` to day-weighted and expose `occupied_desk_days`, `days_in_month`, `occupied_ws_avg` (=desk_days/days, DECIMAL(9,2)) + `occupancy_pct_eom` (old basis, reconciliation) — new columns are view-only, the `t_landlord_*` tables/materialize function are untouched (explicit column lists still match). `silver.landlord_frozen_monthly_occupancy` gains `occupied_ws_avg DECIMAL(9,2) NULL` + `gold.vw_landlord_occupancy_combined` exposes it ([landlord_dashboard_occupancy_freeze_schema.sql](scripts/sql_scripts/landlord_dashboard_occupancy_freeze_schema.sql)); [functions/landlord_freeze_monthly_occupancy.py](functions/landlord_freeze_monthly_occupancy.py) now freezes BOTH bases. `gold.sp_refresh_finance_dashboard` ([core_finance_dashboard_schema.sql](scripts/sql_scripts/core_finance_dashboard_schema.sql)) reads the current month's pct from the contract-book view (ONE implementation of the rule; occupied/vacant stay as-of-today positions; falls back to the position ratio if the view has no row). **Applied to prod** via [scripts/python_scripts/apply_dayweighted_occupancy.py](scripts/python_scripts/apply_dayweighted_occupancy.py) (extracts ONLY the 5 changed batches by marker — NEVER re-run the core_finance file wholesale, its top-level DROP TABLEs would wipe snapshot history; then pre-freezes, rematerializes the 3 t_ tables, EXECs the finance proc, verifies). **Past-data policy (Bryan, explicit): months before 2026-08 keep the old basis forever** — Daniel's Jun-25→May-26 rows and the Jun/Jul-26 cron rows keep `occupied_ws_avg=NULL` and readers fall back to `occupied_workstations`/capacity; **2026-08 was pre-frozen same day with BOTH bases** (`source='manual_override'`, 11 locations) so the Sep-1 04:00 UTC cron (old deployed code, INT-only) skips it via its NOT EXISTS guard. Verified live post-apply: Aug-2026 Zuidtoren 68.29% (old 68.96), Old Street 91.67 (88.10 — mid-month leavers now credited; they counted ZERO before), Chausseestrasse 80.56 (82.53), Fox Court 93.05 (93.55); membership + contract books agree to the row; Flask dashboard history verified (Jul-2026 stays 81.2 old-basis, Aug switches to 68.3). Paired app changes (ai-strategic-partnership repo, same date): `_sql_frozen_occupancy_by_period` returns `{pos, avg}` and the history pct prefers `avg`; donut arc drawn from `occ_pct` (was members/capacity). **NOT done — needs you: deploy = push to `main` BEFORE OCT 1** — until the function app ships the new freeze-cron code, the Oct-1 freeze of September writes no `occupied_ws_avg` and Sep-2026 falls back to the old basis on the charts (recoverable afterwards: UPDATE the frozen row from the view while Sep is still inside the ±12-month spine).)

Previous: 2026-08-31 (**LoopNet — the THIRD payload shape (`srp-ldjson`) + Lusha broker-email tail + grade on buildings-kept.** Symptom: the W36 weekly run showed London **0 buildings** ("we spend our life fixing this"). It was NOT our code and NOT the 2026-08-19 pagination fix — the scrape worked (609 raw items, ≈ W35's 616). **Root cause, proven from `bronze.location_scraper_raw`:** LoopNet's mobile SRP 403'd on every London page this morning, so the memo23 actor fell back to a shape we'd never parsed, `_dataSource: "srp-ldjson"` (it scrapes the search-results page's LD+JSON). Its keys are `propertyId, listingUrl, listingName, description, brokerName, brokerCompany, position` — **no `sizeSf`/`spaces`/`header`, no coordinates, no `address`/`zip`, and no `brokerEmail`.** Surface lives ONLY in `description` ("27,568 sq ft Office Property Offered in …"), address ONLY in `listingName` ("450 Bath Rd, West Drayton UB7 0EB"). The old surface parse (sizeSf / "…SF…Available" name) found nothing → every item failed the ≥1500 m² guardrail → 0 buildings, and the grading (which scores RAW volume) still marked the week `completed`. NB nothing was lost from the map: gold keeps the latest run WITH globe rows, so London kept its 284 W35 markers — the dashboard's 0 came from the run-log table. **Decisive site fact (tested live in a real browser 2026-08-31, bot-check cleared): the broker email is NOT on LoopNet's website at all** — a fully-rendered listing-detail page has 0 emails / 0 phones; contact is a *form* LoopNet relays. The email only ever lived in the mobile API. So **building our own LoopNet scraper cannot get emails** — memo23 dominates it (emails when the mobile API is open, the same emailless HTML we'd scrape when closed); don't build one. The answer is directory + Lusha on the broker's person name + company (**97% of srp-ldjson brokers are a real first+last**, 293/300 London; only ~2% "Sales Enquiries" desks). **Fix (3 changes, py_compile clean; location-scraper suite 155 pass / 7 pre-existing berlin_simulation fails):** (1) **Adapter parses srp-ldjson** — [shared/location_scraper/adapters/loopnet.py](shared/location_scraper/adapters/loopnet.py) `available_surface_sqft_from_payload` adds `description` as the LAST surface source (never overrides `sizeSf`/name — those are tried first, so the healthy `free-mobile-api` path is untouched); `normalize` falls `address` back to `listingName`; [shared/location_scraper/activities/materialize_globe.py](shared/location_scraper/activities/materialize_globe.py) `_map_row` adds `listingName` to its address picks so geocoding has an address. Replay on the real W36 raw: **0 → 323 buildings** (300 distinct), **224 (74%) already emailed via the broker directory**. (2) **Lusha broker-email tail-enrichment** — new `ls_enrich_loopnet_brokers` activity ([shared/location_scraper/activities/enrich.py](shared/location_scraper/activities/enrich.py) `enrich_loopnet_brokers`): LoopNet still skips the agency Lusha fan-out, but the orchestrator now runs a person-based pass — every (broker name, company) the directory can't resolve → `lusha.search_individual` → `extract_best_email` → written into `silver.location_scraper_broker_directory`, which `materialize_globe` reloads THIS run and every future run gets free (the LoopNet broker pool is finite, so it compounds toward ~95%). Directory hits cost nothing; cap `LOOPNET_LUSHA_MAX_LOOKUPS` (default 150); no-op without `LUSHA_API_KEY`. (3) **Grade on buildings-kept** (closes the open gap from 2026-08-19) — in `location_scraper_orch`, `raw_item_count > 0` with `len(listings) == 0` downgrades the run `completed → degraded`, so `ls_cities_needing_run` retries it in the Wednesday pass + the weekly alert names it, instead of banking an empty green week. `== 0` only, so genuinely small cities (Cupertino=5) stay `completed`. **Tests**: +4 srp-ldjson adapter cases (surface from description, address fallback, currency-from-city, below-floor drop, description-never-overrides-sizeSf) in [tests/test_location_scraper_loopnet_adapter.py](tests/test_location_scraper_loopnet_adapter.py); new [tests/test_location_scraper_broker_enrich.py](tests/test_location_scraper_broker_enrich.py) (no-key no-op + lookup cap). **Deploy = push to `main`** (GitHub Actions). No SQL/schema change — the broker directory table already exists. After deploy: London gold shows 224 markers pre-Lusha and climbs as the directory fills; the Wednesday retry re-tries any city the mobile SRP 403'd. **Same-day follow-up — the Lusha tail wrote 0 at first because `search_individual` hit a DEAD endpoint:** the on-demand London run (route is `POST /api/api/scrape` — double `api` prefix from `routePrefix` default + `route="api/scrape"`; the docs said `/api/scrape`) restored 0→300 buildings but the tail wrote nothing. Root cause: the Lusha client POSTed to `/v2/person/search`, which **Lusha retired (404)**, and the 404 was swallowed as "no match" — silently no-op'ing every individual lookup, the LoopNet tail AND the EU sources' `_enrich_individual` path, invisibly, for a while. Fix ([shared/location_scraper/clients/lusha.py](shared/location_scraper/clients/lusha.py)): `search_individual` now calls **`GET /v2/person`** (name/company as query params, api_key in the **HEADER** — a `?api_key=` query param 401s on GET, so `_get` was also fixed to send the header). Verified live (Lizzie Boswell/CBRE, James Womack/Levy, Chris Collins/Strettons → work emails, A+). After the fix the tail on the London run found **40 emails / 61 lookups**, lifting London **72%→92% email (298/323) and gold 217→266 markers**; the 40 `source='lusha'` directory rows are now free forever. Test: [tests/test_location_scraper_lusha_client.py](tests/test_location_scraper_lusha_client.py). **Still open (upstream, memo23's to fix):** the App Check breakage — when the SRP 403s the actor still falls back and small cities can be diluted; the retries mask it, they don't repair it.)

Previous: 2026-08-20 (**Nexudus help desk (customer requests) ingested — hybrid webhook + 15-minute reconciling poll, bronze + silver, backfilled.** Ask was "can we get help desk data, and can Nexudus trigger it so it's live?" **Discovery** (`scripts/python_scripts/inspect_nexudus_helpdesk.py` / `_fit.py`, both kept): the API area is **`support`**, not the `community` the admin nav implies — `support/helpdeskmessages` (**2,888** tickets, 2022-02-28→now, 10 locations, 14 department names, median first response **25 min**), `support/helpdeskcomments` (**4,342** replies, 15 flagged `Internal`), `support/helpdeskdepartments` (**84**, per-location). **Three findings drove the design.** (1) **`UpdatedSince` is silently ignored** on these endpoints — it returns the full table and looks like it worked; the Nexudus-native **`from_{Entity}_UpdatedOn`** is what filters, and it honours minute precision (Aug-04T00:00→83 rows vs T15:00→79). A watermark built on `UpdatedSince` would have looked healthy while doing a full pull every run. (2) **Nexudus has NO update/close/assign webhook event** — all 90 codes in `eWebhookAction` checked; help desk has only **45 `HelDeskMessageCreated`** (Nexudus's own typo) and **46 `HelpDeskCommentCreated`**. So webhook-only would freeze `Closed`/`ClosedOn`/`OwnerId`/`FirstResponseTimeInMinutes` at creation — exactly the SLA fields worth warehousing. Hence hybrid, and the poll is **not** optional. (3) **Those two hooks already existed and had been dead ~3 years**: created 2023-10-18 against `receiver-main.ncg.digital`, last fired 2023-10-23, `ErrorCount=11` → Nexudus auto-disabled them (it kills a hook after 10 consecutive failures) and nobody noticed. They were also scoped to **London Aldgate only** = 55% of tickets. Proof the mechanism does work here: the Zapier `VisitorCheckedin` hook on the **`(beyond Global)`** business (1376491116) fired 2026-08-20 09:05 UTC with 0 errors — so global scoping is the right scope. **Built**: schema [scripts/sql_scripts/nexudus_helpdesk_schema.sql](scripts/sql_scripts/nexudus_helpdesk_schema.sql) (3 bronze + 3 silver, soft-delete columns) — **applied to prod**; 3 pure transformers; 3 bronze writer methods; 3 silver writers; shared registry [shared/nexudus/helpdesk.py](shared/nexudus/helpdesk.py) (endpoints/filters, webhook routing, HMAC verify, `check_webhook_health`) so poll and webhook can't drift; [functions/nexudus_helpdesk_sync.py](functions/nexudus_helpdesk_sync.py) (timer `0 */15 * * * *`, bronze+silver inline, 24h self-healing lookback) and [functions/nexudus_helpdesk_webhook.py](functions/nexudus_helpdesk_webhook.py) (`POST /api/nexudus/helpdesk-webhook`, auth_level=FUNCTION). Registered in `function_app.py`, the nightly fanout, the queue worker `_ENTITY_MAP`, the weekly `nexudus_silver_reconcile`, and `sync_health_report`'s expected-daily set. **Design decisions worth knowing**: the webhook **re-fetches the canonical record by ID** rather than storing the (undocumented) push payload — otherwise two JSON shapes land in one bronze table and the SHA-256 dedup breaks; a comment event **also refreshes its parent ticket**, which fills the comment's location (comment payloads have no BusinessId) and opportunistically captures the lifecycle change a reply usually causes; the handler **answers 200 on almost everything**, deliberately — with the poll re-reading 24h, a dropped delivery is a latency blip, but a 4xx/5xx streak permanently disables the hook, which is the failure that already happened once here. `minutes_to_close` is **derived** (`ClosedOn - CreatedOn`), never copied: the source `MinutesToClose` is computed backwards and returns a negative (`-41.65` on a ticket created 14:46, closed 15:28). **Two defects found and fixed during live testing**: an unknown id took **35 s** because Nexudus answers 500 (not 404) for a dead id and the client's tenacity ladder ran in full while `get_one`'s 404 handler never saw it (wrapped in `RetryError`) → added `RetryError` handling + a `NEXUDUS_WEBHOOK_TIMEOUT_SECONDS` (10 s) ceiling; and a duplicate delivery cost **7.8 s** running two full silver passes to write nothing → `_fetch_and_write` now returns the bronze writer's `changed` flag so replays skip the silver pass (**0.3 s**). **Verified live**: full backfill → **2,840 messages / 4,293 comments / 75 departments** in silver (the 48 dropped messages are exactly the existing `EXCLUDED_LOCATION_SOURCE_IDS` — 20 `(beyond Global)` + 28 closed Kingsbourne House); joins **2840/2840** locations, 2828/2840 coworkers (12 deleted members), 2781/2783 departments, **4293/4293** comment→ticket with **4293/4293** locations inherited; `minutes_to_close` has **0 negatives**; threads reconstruct in order. Incremental steady state measured: **21 rows fetched, 0 written**. Webhook exercised end-to-end with simulated Nexudus POSTs (flat payload, enveloped `{Value:{Id}}`, comment→parent refresh, junk body, dead id) — all 200, idempotent on replay, and the changed-path writes silver in 0.9 s. Tests [tests/test_nexudus_helpdesk.py](tests/test_nexudus_helpdesk.py) **44 pass** (transformers incl. the negative-`MinutesToClose` trap, webhook routing incl. the `HelDesk` typo, signature modes, MERGE parity, registration wiring); full suite 187 run, the only 5 errors are the pre-existing local-venv location-scraper import failures (`pytest`/`apify_client` not installed). **NOT done — needs you**: (a) deploy = push to `main`; (b) in Nexudus, Settings > Integrations > Webhooks, point actions **45 and 46** at `https://func-infinitspace-etl-d0apgnfpcpdjcwdx.northeurope-01.azurewebsites.net/api/nexudus/helpdesk-webhook?code=<function-key>` scoped to **(beyond Global)**, and either delete or re-point the two dead Aldgate-scoped hooks; (c) optionally set `NEXUDUS_WEBHOOK_SECRET` and, once App Insights confirms our HMAC guess matches, flip `NEXUDUS_WEBHOOK_SIGNATURE_MODE=enforce`. **Until (b) is done the poll alone already keeps the data ≤15 min fresh** — no config is required for it to work. I deliberately did not touch the Nexudus webhook configuration, as that is a production change on an external system.)
